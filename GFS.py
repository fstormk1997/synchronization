...
def create_hidden_network(size):
    ...
    for node in (hidden_network):
        p_d = np.random.exponential(L , kout)
        p_d = p_d * np.sqrt(2) * L / max(p_d)
        for dis in p_d:
            length = 10
...
def GFS(v):
    ...
    for i in imag_v:
        for j in i:
            imag.append(j)
    cov = np.cov(real , imag)
    eigenvalue = np.linalg.eig(cov)[0]
    return abs(eigenvalue[0] - eigenvalue[1])/(eigenvalue[0] + eigenvalue[1])
...
Tmax = 10000
rule_count = 9
gfs = [[] for i in range(rule_count)]
dv = 0.02
    
#------------------------------------------------------------------------------input patterns and expected output

Rule = np.loadtxt('Desktop/thesis/results/percent_K/GFS/0.02/xor_rule_paper.txt')
Rule = Rule[:rule_count]
wanted = Rule[:,4]

#------------------------------------------------------------------------------

adaptation(lead_to_critical_state(create_hidden_network(N) , Rule) , Rule)
gfs = np.array(gfs)
a = []
for i in range(len(gfs[0])):
    a.append(np.mean(gfs[:,i]))

f = open('learn.txt' , 'w+')
f.write(str(np.mean(a)))
f.flush()
f.close()
